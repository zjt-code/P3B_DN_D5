...
* Description    :  获取ATT表中的特征Char数据的结构体指针
* Input          :  void
* Output         :  None
...
/*******************************************************************************
*                           陈苏阳@2023-10-19
* Function Name  :  is_measurement_notify
* Description    :  
* Input          :  void
* Output         :  None
* Return         :  uint16_t
...
{
    uint16_t usTemp = 0;
    usTemp = cs_att_db.CgmMeasurementCccdValue[1];
    usTemp = (usTemp << 8) + cs_att_db.CgmMeasurementCccdValue[0];
    //return (ATT_CCC_START_NTF & usTemp);
}


/*******************************************************************************
*                           陈苏阳@2023-10-19
* Function Name  :  is_socp_notify
* Description    :  
* Input          :  void
* Output         :  None
* Return         :  uint16_t
...
{
    uint16_t usTemp = 0;
    usTemp = cs_att_db.CgmSpecificOpsCccdValue[1];
    usTemp = (usTemp << 8) + cs_att_db.CgmSpecificOpsCccdValue[0];
    //return (usTemp & (ATT_CCC_START_NTF | ATT_CCC_START_IND));
}

/*******************************************************************************
*                           陈苏阳@2023-10-19
* Function Name  :  is_racp_notify
* Description    :  
* Input          :  void
* Output         :  None
* Return         :  uint16_t
...
{
	uint16_t usTemp=0;
	usTemp = cs_att_db.CgmRecordAccessControlCccdValue[1];
	usTemp = (usTemp<<8 )+ cs_att_db.CgmRecordAccessControlCccdValue[0];
	//return (usTemp&(ATT_CCC_START_NTF|ATT_CCC_START_IND));
}
...
/*******************************************************************************
*                           陈苏阳@2023-10-19
* Function Name  :  is_status_notify
* Description    :  
* Input          :  void
* Output         :  None
* Return         :  uint16_t
*******************************************************************************/
uint16_t is_status_notify(void)
{
    uint16_t usTemp = 0;
    usTemp = cs_att_db.CgmStatusCccdValue[1];
    usTemp = (usTemp << 8) + cs_att_db.CgmStatusCccdValue[0];
    //return (usTemp & (ATT_CCC_START_NTF | ATT_CCC_START_IND));
}
